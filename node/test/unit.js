...
describe("loginFunction()", function () {
  it("Test login", async function () {
    ...
    ).to.be.equal(await methods.loginFunction("admin", "secret"));
  });
...
describe("protectFunction()", function () {
  ...
  it("Test protected Endpoint with no valid jwt token", function () {
    expect(null).to.be.equal(
      methods.protectFunction(
        "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJyb2xlIjoiYWRtaW4ifQ.StuYX978pQGnCeeaj2E1yBYwQvZIodyDTCJWXdsxBGI_dataextra"
      )
    );
  });
});
